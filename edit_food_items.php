...
<html>
  <head>
    <title> Manager Login | Healthy Hack </title>
  </head>
...
<div class="col-xs-3">
  <div class="form-area" style="padding: 10px 10px 110px 10px; ">
    ...
    $query = mysqli_query($conn, "SELECT * FROM food f WHERE f.R_ID IN (SELECT r.R_ID FROM RESTAURANTS r WHERE r.M_ID='$user_check') ORDER BY food_ID");
    while ($row = mysqli_fetch_array($query)) {
      ?>
      <div class="list-group" style="text-align: center;">
        <?php
      echo "<b><a href='edit_food_items.php?update= {$row['food_ID']}'>{$row['name']}</a></b>";  
        ?>
      </div>
    <?php
    }
